...
$scraper_name = 'DunLaoghaire';
$site_url = 'http://planning.dlrcoco.ie/swiftlg/apas/run';
$crawl_delay = 1;
...
function get_application_urls($date1, $date2, $datefield_prefix = 'reg') {
    ...
    while (true) {
        foreach ($html->find("td[class='apas_tblContent'] a") as $link) {
...
function get_application_details($url) {
    ...
    foreach ($html->find('#fieldset_data') as $field) {
        $key = rtrim(clean_html($field->find('label', 0)->plaintext), ':');
        $app[$key] = clean_html($field->find('p', 0)->plaintext);
    }
    return $app;
}
